...
final class Admin
{
    ...
    public static function guard(): void
    {
        ...
        $hour = gmdate('YmdH');
        $n = $db->prepare('SELECT n FROM admin_failures WHERE ip_hash = ? AND hour = ?');
        $n->execute([$ip, $hour]);
        ...
        if (!hash_equals($token, $given)) {
            $db->prepare('INSERT INTO admin_failures (ip_hash, hour, n) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE n = n + 1')->execute([$ip, $hour]);
            $db->prepare('DELETE FROM admin_failures WHERE hour < ?')->execute([gmdate('YmdH', time() - 86400)]);
    ...
    public static function overview(): array
    {
        ...
        $perEndpoint = $db->prepare("SELECT endpoint, SUM(hits) AS hits FROM usage_counters WHERE bucket LIKE ? GROUP BY endpoint ORDER BY hits DESC");
        $perEndpoint->execute(['d' . $month . '%']);
        $collectors = [];
        foreach (self::COLLECTORS as $name) {
            $last = $db->prepare('SELECT status, message, finished_at FROM collector_runs WHERE name = ? ORDER BY finished_at DESC LIMIT 1');
            $last->execute([$name]);
            $ok = $one("SELECT MAX(finished_at) FROM collector_runs WHERE name = ? AND status = 'ok'", [$name]);
            $collectors[] = ['name' => $name, 'last_run' => $last->fetch() ?: null, 'last_success_at' => $ok ?: null];
        }
        return [
            'users' => (int) $one('SELECT COUNT(*) FROM users'),
            'active_keys' => (int) $one('SELECT COUNT(*) FROM api_keys WHERE revoked_at IS NULL AND expires_at > UTC_TIMESTAMP()'),
            'pending_requests' => (int) $one("SELECT COUNT(*) FROM key_requests WHERE status = 'pending'"),
            'requests_this_month' => (int) $one("SELECT COALESCE(SUM(hits), 0) FROM usage_counters WHERE endpoint = '*' AND bucket = ?", ['M' . $month]),
            'endpoints_this_month' => $perEndpoint->fetchAll(),
            'collectors' => $collectors,
            'switches' => $db->query('SELECT name, disabled, reason, updated_at FROM kill_switch ORDER BY name')->fetchAll(),
        ];
    }

    public static function pending(): array
    {
        return App::db()->query("SELECT id, created_at, name, organization, email, website, usage_description FROM key_requests WHERE status = 'pending' ORDER BY id LIMIT 200")->fetchAll();
    }

    public static function keys(): array
    {
        $st = App::db()->prepare("SELECT k.prefix, u.email, u.plan, u.status AS account_status, k.created_at, k.expires_at, k.revoked_at,
            (SELECT COALESCE(SUM(c.hits), 0) FROM usage_counters c WHERE c.key_id = k.id AND c.endpoint = '*' AND c.bucket = ?) AS month_hits
    ...
    public static function approve(int $id): ?array
    {
        ...
        $uid->execute([$r['email']]);
        $k = Auth::generate();
        $db->prepare('INSERT INTO api_keys (user_id, prefix, key_hash, expires_at, created_at) VALUES (?, ?, ?, UTC_TIMESTAMP() + INTERVAL 2 YEAR, UTC_TIMESTAMP())')
            ->execute([(int) $uid->fetchColumn(), $k['prefix'], Auth::hash($k['key'])]);
        $db->prepare("UPDATE key_requests SET status = 'approved', decided_at = UTC_TIMESTAMP() WHERE id = ?")->execute([$id]);
        $db->commit();
    ...
    {
        $st = App::db()->prepare("UPDATE key_requests SET status = 'rejected', decided_at = UTC_TIMESTAMP() WHERE id = ? AND status = 'pending'");
        $st->execute([$id]);
    ...
    {
        $st = App::db()->prepare('UPDATE api_keys SET revoked_at = UTC_TIMESTAMP() WHERE prefix = ? AND revoked_at IS NULL');
        $st->execute([$prefix]);
        return $st->rowCount();
    }
    ...
    {
        $st = App::db()->prepare('UPDATE users SET status = ? WHERE email = ?');
    ...
    {
        App::db()->prepare('INSERT INTO kill_switch (name, disabled, reason, updated_at) VALUES (?, ?, ?, UTC_TIMESTAMP())
