/** Départements français (code → nom), plus Andorre (99) que la vigilance Météo-France rattache à ses bulletins. */
export const DEPARTEMENTS_FR: Record<string, string> = {
  ...
  '93': 'Seine-Saint-Denis', '94': 'Val-de-Marne', "95": "Val-d'Oise", '99': 'Andorre',
};

const normaliser = (s: string) =>
  s.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();

// Noms normalisés, du plus long au plus court : « Haute-Marne » doit être reconnu avant « Marne ».
const PAR_NOM = Object.entries(DEPARTEMENTS_FR)
  .map(([code, nom]) => ({ code, nom: normaliser(nom) }))
  .sort((a, b) => b.nom.length - a.nom.length);

/** Codes des départements nommés (sans code) dans un texte libre ex. « Cotes d'Armor Finistère Morbihan ». */
export function codesDepuisNoms(texte: string): string[] {
  let reste = ` ${normaliser(texte)} `;
  const trouves = new Set<string>();
  for (const { code, nom } of PAR_NOM) {
    const motif = ` ${nom} `;
    let i: number;
    while ((i = reste.indexOf(motif)) !== -1) {
      trouves.add(code);
      reste = reste.slice(0, i) + ' '.repeat(motif.length - 1) + ' ' + reste.slice(i + motif.length);
    }
  }
  return [...trouves].sort();
}
